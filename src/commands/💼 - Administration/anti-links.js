...
function SaveDBs() { // Fonction pour sauvegarder la base de données
    fs.writeFile("./db.json", JSON.stringify(db, null, 4), (err) => {
        if (err) {
            error_embed.addField(`Une erreur est survenue : `, `${err}`)
...
module.exports = {
    ...
    run: async(client, message, args) => {
        ...
        if(message.member.permissions.has("ADMINISTRATOR")) {
            ...
            if(!arg[1]){
                error_embed.addFields(`${language[guildLang]["ErrorBasic"]}`, `${language[guildLang]["SpecifyConfig3"]}`);
                message.channel.send({embeds : [error_embed]});
                error_embed.fields = [];
                return;
            }else{
                if(arg[1] === "on"){
                        ...
                        antilinks_embed.addField(`${language[guildLang]["AntiLinkStatus"]}`, `${language[guildLang]["Activated"]}`);
                ...
                if(arg[1] === "off"){
                        ...
                        antilinks_embed.setTimestamp();
                        antilinks_embed.addField(`${language[guildLang]["AntiLinkStatus"]}`, `${language[guildLang]["Disabled"]}`);
                        message.channel.send({embeds : [antilinks_embed]});
                        await message.react("✅");
                        antilinks_embed.fields = [];
                }
